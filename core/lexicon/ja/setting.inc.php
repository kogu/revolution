...
$_lang['setting_base_help_url'] = 'ヘルプページの基準URL';
$_lang['setting_base_help_url_desc'] = '管理画面内でページの右上に表示される「ヘルプ」ボタンの、リンク先の基準となるURLを指定して下さい。';
$_lang['setting_blocked_minutes'] = 'ブロックする時間（分）';
$_lang['setting_blocked_minutes_desc'] = 'ユーザーがログイン失敗許可数を超過した場合に、ブロックを行う時間を分数で指定できます。この値は数値のみを入力してください。（カンマやスペース等は使えません）';
...
$_lang['setting_tvs_below_content'] = 'テンプレート変数をcontentの下部に移動';
$_lang['setting_tvs_below_content_desc'] = 'リソースの編集時、テンプレート変数をcontentの下部に移動します。';
$_lang['setting_ui_debug_mode'] = '管理画面UIデバッグモード';
$_lang['setting_ui_debug_mode_desc'] = 'この設定が有効の場合、標準の管理画面テーマに限り、UI処理のデバッグメッセージをconsole.logに出力します。なお、メッセージの出力にはconsole.logに対応したブラウザを使用する必要があります。';
...
$_lang['setting_xhtml_urls_desc'] = 'この設定が有効の場合、MODXが生成する全てのURLで、URLに含まれるアンパーサンド (&) を自動的に文字実態参照 (&amp;) に置き換えます。';